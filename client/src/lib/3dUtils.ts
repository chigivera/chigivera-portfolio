...
// Create a camera
export const createCamera = (aspectRatio: number, fov = 75, near = 0.1, far = 1000): THREE.PerspectiveCamera => {
...
export const createLaptopModel = (screenTexture?: THREE.Texture): THREE.Group => {
  ...
  // Add keyboard keys for more detail
  const keysGroup = new THREE.Group()
  const keySize = 0.15
  const keySpacing = 0.02
  const keyRows = 4
  const keyCols = 10
  const keyGeometry = new THREE.BoxGeometry(keySize, keySize, 0.05)
  ...
  for (let row = 0; row < keyRows; row++) {
    for (let col = 0; col < keyCols; col++) {
      ...
      const xPos = (col - keyCols / 2 + 0.5) * (keySize + keySpacing)
      const yPos = (row - keyRows / 2 + 0.5) * (keySize + keySpacing)
      key.position.set(xPos, 0.03, yPos)
      keysGroup.add(key)
    }
  }
  ...
  touchpad.position.set(0, 0.02, 0.7)
  touchpad.rotation.x = -Math.PI / 2
  keysGroup.add(touchpad)

  base.add(keysGroup)

  // Add laptop logo on back of screen
  ...
  for (let l = 0; l < layers.length; l++) {
    ...
    if (l > 0) {
      const prevLayerNodes = nodes.slice(
        nodes.length - layerNodes.length - layers[l - 1],
  ...
  for (let i = 0; i < positions.length; i += 3) {
    ...
    if (mousePosition) {
      const distance = Math.sqrt(
        Math.pow(positions[i] - mousePosition.x, 2) + Math.pow(positions[i + 1] - mousePosition.y, 2),
      )
